...
const Dashboard = ({authedUser, questions, users}) => {
  ...
  const filteredAns = questions.filter(answered);
  const filteredUnAns = questions.filter(unanswered);
    ...
    <>
      ...
      >
        <Tab eventKey="unanswered" title="New Polls">
          <Row>
            {filteredUnAns && filteredUnAns.map((poll) => (
        ...
        <Tab eventKey="answered" title="Answered Polls">
          <Row>
            {filteredAns && filteredAns.map(poll => (
                <Col xs="12" lg="3" className="mb-3" key={poll.id}>
                  <Card poll={poll} author={users[poll.author]} />
                </Col>
            ))}
